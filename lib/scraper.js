...
export const getAmazonBestsellerUrls = () => {
    ...
    return new Promise((resolve) => {
        ...
        .then((subCategoriesOfSubCategories) => {
            ...
            return Promise.all(subCategoriesOfSubCategories.slice(0,3).map((subCategoryArraySubCategories) => {

                return Promise.all(subCategoryArraySubCategories.slice(0,3).map((subSubCategoryItems) => {

                    return Promise.all(subSubCategoryItems.slice(0,3).map((subSubCategory) => {

                        // collected.push(`3: ${subSubCategory.parent}/${subSubCategory.title}`);
        ...
        .then((subCategoriesOfSubCategories) => {
            ...
            return Promise.all(subCategoriesOfSubCategories.slice(0,3).map((subCategoryArraySubCategories) => {
                ...
                return Promise.all(subCategoryArraySubCategories.slice(0,3).map((subSubCategoryItems) => {
                    ...
                    return Promise.all(subSubCategoryItems.slice(0,3).map((subSubCategoryArray) => {
                        return subSubCategoryArray.slice(0,3).map((subSubCategory) => {
                            // console.log('SSC:', subSubCategory);
                            // collected.push(`4: ${subSubCategory.parent}/${subSubCategory.title}`);
                            urls.push(subSubCategory.url);
                        });
        ...
        .then(() => {
            require('fs').writeFileSync('urls.json', JSON.stringify(urls, null, 2), { encoding: 'utf-8'});
